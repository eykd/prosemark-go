# Data Model: Prosemark Binder v1

**Branch**: `001-prosemark-binder` | **Date**: 2026-02-23

All types live in `internal/binder/types.go` unless noted.
...
Represents one structural node in the binder tree. The synthetic root has `Type = "root"` and no
`Target` or `Title`. All other nodes have `Type = "node"`.
...
Structured error or warning record emitted during parse or operations.
...
External project file map supplied by the caller.
...
FileRef     ::= RelativePath | BareStem
RelativePath::= PathComponent "/" PathComponent ( "/" PathComponent )*
PathComponent ::= [^/\[\]:.]+
BareStem    ::= [^/\[\]:.]+
Index       ::= [0-9]+
```
...
Project ────────────────────────── supplies ──────► Parser
                       ...
                       raw binder bytes ──────────────┤
                                                ...
                                                ├── Lines: []string (source preservation)
                                                ├── LineEnds: []string
                                                └── RefDefs: map[string]RefDef

ParseResult ──► Selector Engine ──► SelectorResult ──► Op Functions
                                                             │
                                                             ▼
                                                    Modified []byte
                                                         +
...
## State Transitions (Mutation Operations)

```
Input binder bytes
        │
        ▼
   Parse → ParseResult + parse diagnostics
        │
        ├─[OPE009 on parse error]──► abort; return unchanged bytes
        │
        ▼
   Validate preconditions (selector eval, cycle detect, etc.)
        │
        ├─[any OPExx]─────────────► abort; return unchanged bytes + diagnostics
        │
        ▼
   Splice lines → modified []byte
        │
        ▼
   Write to disk
        │
        ├─[IO error]───────────────► OPE009; return unchanged bytes
        │
        ▼
   OpResult{Changed: true, Diagnostics: merged}
```
